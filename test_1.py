...
def readwav(filename):
    spf = wave.open('M:/AITest copy/New folder/17913-4-0-1.wav', 'r')
...
def plot_stft(X, tonic, title):
    ...
    plt.title(title)
    plt.title('SFTF color')
    plt.show()


files = ['M:/AITest copy/New folder/17913-4-0-1.wav']
ROOT = ''
...
for f in files:
    ...
    for line in [str(y) + ";" + str(x) + "\n" for (x, y) in zip(FFT.tolist(), freqs.tolist())]:
        f.write(line)
    f.close()
    plot_fft(FFT, freqs, num_samples, 110, 2000, f)  # 220+56
    plt.show()
